...
# Used only for the optional training-range table.
# =========================================================
...
# Used only to check whether inputs are within training ranges.
# =========================================================
REFERENCE_DATA = None
DATA_ERROR = None
...
try:
    if DATA_PATH.exists():
        ...
        if missing_columns:
            raise ValueError(
                "Missing columns in Raw_Data_Set.xlsx: "
                f"{missing_columns}"
            )
...
# PREDICTION FUNCTION
#
# ZeroGPU requires at least one function decorated with
# @spaces.GPU. ExtraTrees itself still performs CPU-based
# prediction.
# =========================================================
...
def predict_axial_frequency(
    ...
    try:
        ...
        if REFERENCE_DATA is None:
            if DATA_ERROR:
                status_message = (
                    "ℹ️ Prediction completed, but the "
                    "training-range check could not be performed. "
                    f"Dataset error: {DATA_ERROR}"
                )
            else:
                status_message = (
                    "ℹ️ Prediction completed. "
                    "Raw_Data_Set.xlsx was not found, so the "
                    "training-range check was skipped."
                )

        elif outside_count > 0:
            status_message = (
                f"⚠️ **Warning:** {outside_count} input "
                "value(s) are outside the training-data range. "
                "The prediction may be less reliable because "
                "it involves extrapolation."
            )
        ...
        return (
            error_html,
            "Please check the inputs and Space logs.",
            empty_table,
        )
